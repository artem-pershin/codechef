...
public class GUESSPRM2 {
        ...
        private long question() {
            ...
            if (M == 1) {
                ...
                try {
                    ...
                    System.out.println(answers + " prime: " + answer);
                    throw e;
                }
            }
    ...
    private static void reallyLargeTest() throws IOException {
        int primeNum = 50847534;

        BufferedReader br = new BufferedReader(new FileReader("/Users/temych/Documents/primes.txt"));

        int[] primesLarge = new int[primeNum];

        int i = 0;
        String line;
        while ((line = br.readLine()) != null) {
            primesLarge[i++] = Integer.parseInt(line);
        }

        for (int j = 0; j < primeNum; j++) {
            Oracle o = new Oracle(primesLarge[j]);

            if (j % 10_000 == 0) {
                System.out.println(String.format("%d processed", j));
            }

            PrimeGuesser guesser = new PrimeGuesser(primesLarge[j]);
